...
public class Pr09 {
	...
	public static void main(String[] args) {
		...
		/*
		 * []                     : 범위 (범위에 해당하는 문자 하나)
		 * .                      : 모든 문자(공백, 한글, 숫자, 영문, 특수문자 등의 문자 하나) (줄 끝부분과 일치하거나 일치하지 않을 수 있음)
		 * ?=.*[조건]         	  : 긍정형 전방탐색 (positive lookahead) -> 일치하는 문자가 있으면 값이 아닌 인덱스를 반환 -> 조건에 맞는 문자 포함 여부를 확인하는 것
		 *               ==>  ?= : 긍정형 전방탐색 (positive lookahead), 값이 아닌 인덱스를 반환
		 *               	   . : 모든 문자(줄 끝부분과 일치하거나 일치하지 않을 수 있음)
		 *               	   * : 몇 개가 오든 상관 X
		 *                   [ ] : 범위
		 *
		 * 1. ^                   : 시작
		 ...
		 * 	  2번 ~ 4번을 합치면 ==> 영문, 숫자, 특수문자($ 제외)가 하나 이상 포함되어 있음을 확인하는 구문
		 * 5. [a-zA-Z0-9!@#%^&*]  : 소문자, 대문자, 숫자, 특수문자($ 제외) 범위
		 * 6. [^$]                : $ 제외
